...
nb_gtws = 68
gtws = ["BS {}".format(i) for i in range(1,nb_gtws+1)]
#gtws = gateway_list_track(db.request_track(21,0,7,'ALL',500))

dataset_antwerp = pd.read_csv("lorawan_dataset_antwerp.csv")
clustering_database = db.transform_antwerp(dataset_antwerp)
print("{} points imported from Antwerp dataset".format(len(clustering_database)))
...
mapping.output_map('maps/clustering-map-antwerp.html')
